...
export default function PagamentosPage() {
  ...
  const totalBruto  = summary.reduce((s, f) => s + (Number(f.bruto) || 0), 0)
  const totalPago   = summary.reduce((s, f) => s + (Number(f.pago) || 0) + (Number(f.descontos) || 0), 0)
  const totalAberto = summary.reduce((s, f) => s + Math.max(0, Number(f.saldo) || 0), 0)
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="grid grid-cols-3 gap-4">
        {[{l:'Total Bruto',v:totalBruto,c:'text-white'},{l:'Total Pago',v:totalPago,c:'text-green-400'},{l:'Em Aberto',v:totalAberto,c:'text-red-400'}].map(({l,v,c})=>(
          <div key={l} className="card"><p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">{l}</p><p className={`text-2xl font-extrabold ${c}`}>{fmtR$(v)}</p></div>
        ))}
      </div>
        ...
        {summary.length === 0 ? <p className="text-gray-400 text-center py-8">Nenhum funcionário ativo</p> : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-gray-800">{['Funcionário','Dias','Bruto','Descontos','Líquido','Pago','Saldo',''].map(h=><th key={h} className="text-left py-3 px-4 text-xs font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead>
              <tbody>
                {summary.map((s, i) => (
                  <tr key={s.employee.id} className={`border-b border-gray-800 hover:bg-gray-800/50 ${i%2===1?'bg-gray-800/20':''}`}>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${(Number(s.saldo)||0)>0?'bg-yellow-500/20 text-yellow-400':'bg-green-500/20 text-green-400'}`}>{s.employee.nome.slice(0,2).toUpperCase()}</div>
                        <div><p className="text-white text-sm font-medium">{s.employee.nome}</p><p className="text-gray-500 text-xs">{s.employee.funcao}</p></div>
                      </div>
                    </td>
                    <td className="py-3 px-4 text-gray-300 text-sm">{s.dias||0}</td>
                    ...
                    <td className="py-3 px-4 text-blue-400 text-sm">{fmtR$(s.liquido)}</td>
                    <td className="py-3 px-4 text-green-400 text-sm">{fmtR$(s.pago)}</td>
                    <td className="py-3 px-4"><span className={`font-bold text-sm ${(Number(s.saldo)||0)>0?'text-red-400':'text-green-400'}`}>{fmtR$(s.saldo)}</span></td>
                    <td className="py-3 px-4"><button onClick={()=>{setForm(p=>({...p,employee_id:s.employee.id,valor:String(Math.max(0,Number(s.saldo)||0))}));setModal(true)}} className="btn-primary text-xs py-1 px-3">Registrar</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div onClick={e=>{if(e.target===e.currentTarget)setModal(false)}} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <div className="p-5 space-y-4">
              <div><label className="label">Funcionário *</label>
                <select value={form.employee_id} onChange={up('employee_id')} className="input-field">
                  ...
                  {summary.map(s=><option key={s.employee.id} value={s.employee.id}>{s.employee.apelido||s.employee.nome} → saldo: {fmtR$(s.saldo)}</option>)}
                </select>
              </div>
              ...
              <div><label className="label">Tipo</label>
                <select value={form.tipo} onChange={up('tipo')} className="input-field">
                  {['Semanal','Quinzenal','Mensal','Adiantamento','Acerto Final'].map(t=><option key={t}>{t}</option>)}
                </select>
              </div>
            ...
            <div className="flex justify-end gap-3 p-5 border-t border-gray-800">
              ...
              <button onClick={salvar} disabled={saving} className="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2 rounded-lg text-sm">{saving?'Salvando...':'✓ Confirmar'}</button>
            </div>
          </div>
        </div>
      )}
