...
export default function RelatoriosPage() {
  ...
  const barData = summary.slice(0,8).map(s=>({name:s.employee.apelido||s.employee.nome.split(' ')[0],Bruto:s.bruto||0,Descontos:s.descontos||0,Pago:s.pago||0}))
  ...
  const exportarExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([
      ...
      ...summary.map(s=>[s.employee.nome,s.dias||0,s.faltas||0,(s.horasOutro||0).toFixed(1),s.bruto||0,s.descontos||0,s.liquido||0,s.pago||0,s.saldo||0,s.inss||0,s.fgts||0])
    ]),'Resumo')
    XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([
      ['Funcionário','Data','Valor','Tipo','Obs'],
      ...payments.filter(p=>!fF||p.employee_id===fF).map(p=>[p.employee_name,fmtBR(p.data),p.valor||0,p.tipo,p.obs||''])
    ]),'Pagamentos')
    XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([
      ['Funcionário','Mês','Ano','INSS','FGTS','Total'],
      ...taxasFilt.map(t=>{const emp=employees.find(e=>e.id===t.employee_id);return[emp?.nome||'',MESES[t.month-1],t.year,t.inss_value||0,t.fgts_value||0,(Number(t.inss_value)||0)+(Number(t.fgts_value)||0)]})
    ]),'Encargos')
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex justify-between items-center">
        <div><h1 className="text-2xl font-extrabold text-white mb-1">Relatórios</h1><p className="text-gray-400 text-sm">{lista.length} registros</p></div>
      ...
      {aba==='empresa'&&(
        <div className="space-y-5">
          <div className="grid grid-cols-4 gap-4">
            {[{l:'Total Bruto',v:fmtR$(totBruto),c:'text-white'},{l:'Total Pago',v:fmtR$(totPago),c:'text-green-400'},{l:'Em Aberto',v:fmtR$(totSaldo),c:'text-red-400'},{l:'Encargos',v:fmtR$(totEncargos),c:'text-purple-400'}].map(({l,v,c})=><div key={l} className="card"><p className="text-xs font-semibold text-gray-400 uppercase mb-2">{l}</p><p className={`text-xl font-extrabold ${c}`}>{v}</p></div>)}
          </div>
          <div className="grid grid-cols-4 gap-4">
            {[{l:'Faltas',v:totFaltas,c:'text-red-400'},{l:'H.Personaliz.',v:totHOutro.toFixed(1)+'h',c:'text-yellow-400'},{l:'INSS',v:fmtR$(totINSS),c:'text-blue-400'},{l:'FGTS',v:fmtR$(totFGTS),c:'text-cyan-400'}].map(({l,v,c})=><div key={l} className="card"><p className="text-xs font-semibold text-gray-400 uppercase mb-2">{l}</p><p className={`text-xl font-extrabold ${c}`}>{v}</p></div>)}
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="card"><h3 className="text-white font-bold mb-4">Diárias — 7 dias</h3>
              <ResponsiveContainer width="100%" height={180}><AreaChart data={dias7}><defs><linearGradient id="gB" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.3}/><stop offset="95%" stopColor="#3b82f6" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#1f2937"/><XAxis dataKey="name" tick={{fill:'#6b7280',fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:'#6b7280',fontSize:11}} axisLine={false} tickLine={false}/><Tooltip contentStyle={{background:'#111827',border:'1px solid #1f2937',borderRadius:'8px',fontSize:'12px'}}/><Area type="monotone" dataKey="Bruto" stroke="#3b82f6" fill="url(#gB)" strokeWidth={2}/></AreaChart></ResponsiveContainer>
            </div>
          ...
          <div className="card"><h3 className="text-white font-bold mb-4">Resumo Geral</h3>
            <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-800">{['Funcionário','Dias','Faltas','H.Outro','Bruto','Descontos','Líquido','Pago','Saldo','INSS','FGTS'].map(h=><th key={h} className="text-left py-3 px-3 text-xs font-semibold text-gray-400 uppercase whitespace-nowrap">{h}</th>)}</tr></thead>
              <tbody>{summary.map((s,i)=><tr key={s.employee.id} className={`border-b border-gray-800 hover:bg-gray-800/50 ${i%2===1?'bg-gray-800/20':''}`}>
                <td className="py-2 px-3 text-white font-medium text-sm">{s.employee.nome}</td>
                <td className="py-2 px-3 text-green-400 text-sm">{s.dias||0}</td>
                <td className="py-2 px-3 text-red-400 text-sm">{s.faltas||0}</td>
                <td className="py-2 px-3 text-yellow-400 text-sm">{(s.horasOutro||0).toFixed(1)}h</td>
                <td className="py-2 px-3 text-white text-sm">{fmtR$(s.bruto)}</td>
                <td className="py-2 px-3 text-yellow-400 text-sm">{fmtR$(s.descontos)}</td>
                <td className="py-2 px-3 text-blue-400 text-sm">{fmtR$(s.liquido)}</td>
                <td className="py-2 px-3 text-green-400 text-sm">{fmtR$(s.pago)}</td>
                <td className="py-2 px-3"><span className={`font-bold text-sm ${(s.saldo||0)>0?'text-red-400':'text-green-400'}`}>{fmtR$(s.saldo)}</span></td>
                <td className="py-2 px-3 text-blue-300 text-sm">{fmtR$(s.inss)}</td>
                <td className="py-2 px-3 text-cyan-400 text-sm">{fmtR$(s.fgts)}</td>
              </tr>)}</tbody>
              <tfoot><tr className="border-t-2 border-gray-700 bg-gray-800/40">
                <td className="py-2 px-3 text-white font-bold text-sm">TOTAL</td>
                <td className="py-2 px-3 text-green-400 font-bold">{summary.reduce((s,f)=>s+(f.dias||0),0)}</td>
                <td className="py-2 px-3 text-red-400 font-bold">{totFaltas}</td>
                <td className="py-2 px-3 text-yellow-400 font-bold">{totHOutro.toFixed(1)}h</td>
                <td className="py-2 px-3 text-white font-bold">{fmtR$(totBruto)}</td>
                <td className="py-2 px-3 text-yellow-400 font-bold">{fmtR$(totDesc)}</td>
                <td className="py-2 px-3 text-blue-400 font-bold">{fmtR$(totBruto-totDesc)}</td>
                <td className="py-2 px-3 text-green-400 font-bold">{fmtR$(totPago)}</td>
                <td className="py-2 px-3 text-red-400 font-bold">{fmtR$(totSaldo)}</td>
                <td className="py-2 px-3 text-blue-300 font-bold">{fmtR$(totINSS)}</td>
                <td className="py-2 px-3 text-cyan-400 font-bold">{fmtR$(totFGTS)}</td>
              </tr></tfoot>
            </table></div>
          </div>
        </div>
      ...
      {aba==='jornadas'&&(
        <div className="space-y-5">
          ...
          {listaAusencias.length>0&&(<div className="card"><div className="flex items-center gap-2 mb-3"><AlertCircle size={15} className="text-red-400"/><h3 className="text-white font-bold">Faltas e Ausências ({listaAusencias.length})</h3></div>
            <table className="w-full"><thead><tr className="border-b border-gray-800">{['Funcionário','Data','Motivo','Obs'].map(h=><th key={h} className="text-left py-2 px-3 text-xs font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead>
            <tbody>{listaAusencias.map((r,i)=><tr key={r.id} className={`border-b border-gray-800 ${i%2===1?'bg-gray-800/20':''}`}><td className="py-2 px-3 text-white text-sm">{r.employee_name}</td><td className="py-2 px-3 text-gray-400 text-sm">{fmtBR(r.data)}</td><td className="py-2 px-3"><span className="badge-red">{r.absence_reason||'Sem motivo'}</span></td><td className="py-2 px-3 text-gray-400 text-sm">{r.obs||'—'}</td></tr>)}</tbody></table>
          </div>)}
          {listaOutro.length>0&&(<div className="card"><h3 className="text-white font-bold mb-3">Horas Personalizadas ({listaOutro.length})</h3>
            <table className="w-full"><thead><tr className="border-b border-gray-800">{['Funcionário','Data','Entrada','Saída','Horas','Diária','Obs'].map(h=><th key={h} className="text-left py-2 px-3 text-xs font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead>
            <tbody>{listaOutro.map((r,i)=><tr key={r.id} className={`border-b border-gray-800 ${i%2===1?'bg-gray-800/20':''}`}><td className="py-2 px-3 text-white text-sm">{r.employee_name}</td><td className="py-2 px-3 text-gray-400 text-sm">{fmtBR(r.data)}</td><td className="py-2 px-3 text-gray-300 text-sm">{r.entrada||'—'}</td><td className="py-2 px-3 text-gray-300 text-sm">{r.saida||'—'}</td><td className="py-2 px-3"><span className="badge-yellow">{Number(r.horas)||0}h</span></td><td className="py-2 px-3 text-green-400 text-sm">{fmtR$(r.diaria)}</td><td className="py-2 px-3 text-gray-400 text-sm">{r.obs||'—'}</td></tr>)}</tbody></table>
          </div>)}
          <div className="card"><h3 className="text-white font-bold mb-3">Todos os Registros ({lista.length})</h3>
            <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-800">{['Funcionário','Data','Jornada','Entrada','Saída','Horas','Diária','Desconto','Obs'].map(h=><th key={h} className="text-left py-2 px-3 text-xs font-semibold text-gray-400 uppercase whitespace-nowrap">{h}</th>)}</tr></thead>
              <tbody>{lista.map((r,i)=>{const cfg=JORNADA_CONFIG[r.jornada]||JORNADA_CONFIG.DIA_INTEIRO;const desc=Number(r.discount_value)||Number(r.valor_vale)||0;const isAus=r.jornada==='NAO_TRABALHOU';return(
                <tr key={r.id} className={`border-b border-gray-800 ${i%2===1?'bg-gray-800/20':''} ${isAus?'opacity-60':''}`}>
                  <td className="py-2 px-3 text-white text-sm">{r.employee_name}</td>
                  <td className="py-2 px-3 text-gray-400 text-sm">{fmtBR(r.data)}</td>
                  ...
                  <td className="py-2 px-3 text-gray-400 text-xs max-w-[140px] truncate">{r.absence_reason||r.obs||'—'}</td>
                </tr>
              )})}</tbody>
            </table></div>
          </div>
      ...
      {aba==='financeiro'&&(
        <div className="space-y-5">
          ...
          <div className="card"><h3 className="text-white font-bold mb-4">Saldo por Funcionário</h3>
            <table className="w-full"><thead><tr className="border-b border-gray-800">{['Funcionário','Bruto','Descontos','Líquido','Pago','Saldo'].map(h=><th key={h} className="text-left py-3 px-4 text-xs font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead>
          ...
          <div className="card"><h3 className="text-white font-bold mb-4">Pagamentos</h3>
            <table className="w-full"><thead><tr className="border-b border-gray-800">{['Funcionário','Data','Valor','Tipo','Obs'].map(h=><th key={h} className="text-left py-3 px-4 text-xs font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead>
            <tbody>{payments.filter(p=>!fF||p.employee_id===fF).map((p,i)=><tr key={p.id} className={`border-b border-gray-800 hover:bg-gray-800/50 ${i%2===1?'bg-gray-800/20':''}`}>
              <td className="py-3 px-4 text-white text-sm">{p.employee_name}</td>
            ...
            return(
              <div key={s.employee.id} className="card space-y-4">
                <div className="flex items-center justify-between pb-4 border-b border-gray-800">
                  ...
                  <div className="text-right"><p className="text-gray-400 text-xs mb-1">Saldo</p><p className={`text-2xl font-extrabold ${(s.saldo||0)>0?'text-red-400':'text-green-400'}`}>{fmtR$(s.saldo)}</p></div>
                </div>
                <div className="grid grid-cols-4 gap-3">{[{l:'Dias',v:s.dias||0,c:'text-green-400'},{l:'Faltas',v:s.faltas||0,c:'text-red-400'},{l:'Dia Inteiro',v:s.diasInteiros||0,c:'text-blue-400'},{l:'Meio Turno',v:s.meiosTurnos||0,c:'text-yellow-400'}].map(({l,v,c})=><div key={l} className="bg-gray-800/50 rounded-lg p-3"><p className="text-xs text-gray-400 mb-1">{l}</p><p className={`font-bold text-xl ${c}`}>{v}</p></div>)}</div>
                ...
                {outros.length>0&&(<div><h4 className="text-yellow-400 font-semibold text-sm mb-2">⏱ Horas Personalizadas ({outros.length})</h4><div className="space-y-1">{outros.map(r=><div key={r.id} className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg px-3 py-2 flex items-center gap-4 flex-wrap"><span className="text-gray-300 text-sm">{fmtBR(r.data)}</span><span className="text-yellow-400 text-sm">{r.entrada||'?'}→{r.saida||'?'} ({Number(r.horas)||0}h)</span><span className="text-green-400 text-sm">{fmtR$(r.diaria)}</span>{r.obs&&<span className="text-gray-400 text-xs italic">{r.obs}</span>}</div>)}</div></div>)}
                {empPags.length>0&&(<div><h4 className="text-white font-semibold text-sm mb-2">Pagamentos</h4><div className="space-y-1">{empPags.map(p=><div key={p.id} className="bg-green-500/10 border border-green-500/20 rounded-lg px-3 py-2 flex items-center gap-4"><span className="text-gray-300 text-sm">{fmtBR(p.data)}</span><span className="text-green-400 font-bold text-sm">{fmtR$(p.valor)}</span><span className="badge-blue">{p.tipo}</span></div>)}</div></div>)}
