...
const defaultData = {
  ...
  empresa_apresentacao:'A Nascimento Pinturas e especializada em pinturas prediais com mais de 20 anos de experiencia em Bento Goncalves e regiao. Contamos com equipe propria, qualificada e registrada, atuando de forma totalmente legalizada e em conformidade com a CLT e as exigencias do Ministerio do Trabalho.',
  ...
  validade_dias:30, prazo_execucao:'3 (tres) meses',
...
function parseLinhas(texto: string): Omit<Item,'id'>[] {
  return texto.split('\n')
    ...
    .filter(l => l.length > 2)
    .map(l => {
      const match = l.match(/^(\d+[.,]?\d*)\s*(m2|m|un|lt|bd|gl|pc|mt|hr|kg|vb)?\s+(.+)/i)
      if (match) {
        return {
  ...
  return (
    <div style={FS}>
      ...
      <div onClick={() => ref.current?.click()} style={{ border:'1px dashed var(--border)', borderRadius:8, padding:10, textAlign:'center', cursor:'pointer', background:'var(--bg-hover)', minHeight:h, display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden', position:'relative' }}>
        ...
        {src && <div style={{ position:'absolute', top:4, right:4, background:'rgba(0,0,0,0.5)', borderRadius:4, padding:'1px 5px', fontSize:9, color:'#fff', cursor:'pointer' }} onClick={e=>{e.stopPropagation();onFile('')}}>x</div>}
      </div>
...
function ModalImportar({ onClose, onImportar, onAdicionar }: {
  ...
  const itensSel = parsed.filter((_,i) => selecionados.has(i))
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, width:'100%', maxWidth:580, maxHeight:'88vh', display:'flex', flexDirection:'column', overflow:'hidden' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 18px', borderBottom:'1px solid var(--border)' }}>
          ...
          <button onClick={onClose} style={{ background:'none', border:'none', color:'var(--text-muted)', cursor:'pointer', fontSize:18, lineHeight:1 }}>x</button>
        </div>
          ...
          {step==='colar' ? (
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              <p style={{ fontSize:12, color:'var(--text-muted)', lineHeight:1.6 }}>
                Cole aqui o texto copiado do Word. Cada linha sera um item.
                Voce pode incluir quantidade e unidade no inicio da linha:<br/>
                <span style={{ fontFamily:'monospace', fontSize:11, background:'var(--bg-hover)', padding:'2px 6px', borderRadius:4 }}>10 lt Tinta acrilica premium</span>
              </p>
              <textarea
                value={texto}
                onChange={e => setTexto(e.target.value)}
                placeholder={'10 lt Tinta acrilica premium\n5 lt Selador acrilico\nMassa corrida PVA\n20 kg Impermeabilizante'}
                style={{ ...IS, minHeight:200, resize:'vertical', fontFamily:'monospace', fontSize:12 }}
                autoFocus
              />
              <p style={{ fontSize:11, color:'var(--text-muted)' }}>{nLinhas} linhas detectadas</p>
            </div>
          ) : (
            <div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
                ...
                <div style={{ display:'flex', gap:6 }}>
                  <button onClick={()=>setSelecionados(new Set(parsed.map((_,i)=>i)))} style={{ fontSize:11, padding:'3px 8px', borderRadius:6, border:'1px solid var(--border)', background:'none', color:'var(--text-muted)', cursor:'pointer' }}>Todos</button>
                  <button onClick={()=>setSelecionados(new Set())} style={{ fontSize:11, padding:'3px 8px', borderRadius:6, border:'1px solid var(--border)', background:'none', color:'var(--text-muted)', cursor:'pointer' }}>Nenhum</button>
                </div>
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
                {parsed.map((item, i) => (
                  <div key={i} onClick={()=>toggleItem(i)} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', borderRadius:8, background:selecionados.has(i)?'rgba(96,165,250,0.08)':'var(--bg-hover)', border:selecionados.has(i)?'1px solid rgba(96,165,250,0.3)':'1px solid var(--border)', cursor:'pointer' }}>
                    <div style={{ width:16, height:16, borderRadius:4, background:selecionados.has(i)?'#60a5fa':'transparent', border:selecionados.has(i)?'1px solid #60a5fa':'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      {selecionados.has(i) && <span style={{ color:'#fff', fontSize:10, fontWeight:700 }}>v</span>}
                    </div>
                    <div style={{ flex:1 }}>
        ...
        <div style={{ padding:'12px 18px', borderTop:'1px solid var(--border)', display:'flex', gap:8, justifyContent:'flex-end' }}>
          {step==='colar' ? (
            <>
              ...
              <button onClick={processar} disabled={nLinhas===0} style={{ padding:'7px 16px', borderRadius:8, background:'var(--text-primary)', color:'var(--bg-primary)', border:'none', cursor:'pointer', fontSize:13, fontWeight:600, opacity:nLinhas>0?1:0.5 }}>
                Avancar
              </button>
            </>
          ) : (
            <>
              ...
              <button onClick={()=>{onAdicionar(itensSel);onClose()}} disabled={selecionados.size===0} style={{ padding:'7px 14px', borderRadius:8, border:'1px solid var(--border)', background:'none', color:'var(--text-primary)', cursor:'pointer', fontSize:13, opacity:selecionados.size>0?1:0.5 }}>
                + Adicionar a lista
              </button>
              <button onClick={()=>{onImportar(itensSel);onClose()}} disabled={selecionados.size===0} style={{ padding:'7px 16px', borderRadius:8, background:'var(--text-primary)', color:'var(--bg-primary)', border:'none', cursor:'pointer', fontSize:13, fontWeight:600, opacity:selecionados.size>0?1:0.5 }}>
  ...
  return (
    <div style={{ border:'1px solid var(--border)', borderRadius:10, overflow:'hidden', marginBottom:10 }}>
      <div onClick={()=>setOpen(o=>!o)} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', background:'var(--bg-primary)', cursor:'pointer' }}>
        <span style={{ fontSize:13, fontWeight:600, color:'var(--text-primary)' }}>{sec.titulo||'Nova secao'}</span>
      ...
      {open && (
        <div style={{ padding:14, display:'flex', flexDirection:'column', gap:10 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>
            <Field label="Titulo"><input value={sec.titulo} onChange={e=>up('titulo',e.target.value)} style={IS} placeholder="Ex: Alvenaria Externa"/></Field>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            <Field label="Valor mao de obra (R$)"><input type="number" value={sec.valor_mao_obra} onChange={e=>up('valor_mao_obra',+e.target.value)} style={IS}/></Field>
  ...
  return (
    <div style={{ border:'1px solid var(--border)', borderRadius:10, padding:14, marginBottom:8, display:'grid', gridTemplateColumns:'1fr 130px', gap:10 }}>
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        ...
        <Field label="Descricao"><textarea value={pat.descricao} onChange={e=>onChange({...pat,descricao:e.target.value})} style={{ ...TA, minHeight:56 }} placeholder="Descreva o problema"/></Field>
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
...
export default function OrcamentoFormPage() {
  ...
  const TABS = [
    ...
    { id:'patologias', label:'Diagnostico' },
    { id:'secoes',     label:'Servicos' },
  ...
  return (
    <div style={{ padding:20, background:'var(--bg-primary)', minHeight:'100vh' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div>
            <h1 style={{ fontSize:17, fontWeight:700, color:'var(--text-primary)', letterSpacing:'-.3px' }}>
              {isEdit ? 'Editando: '+data.numero : 'Novo Orcamento'}
            </h1>
      ...
      <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:12, padding:18 }}>
        ...
        {tab==='fotos' && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:14 }}>
              ...
              <div style={FS}>
                <label style={LS}>Marca dagua das paginas</label>
                <ImgUpload label="" src={marcaDagua} onFile={setMarcaDagua}/>
                <p style={{ fontSize:10, color:'var(--text-muted)', marginTop:2 }}>Aparece suavemente no fundo de todas as paginas</p>
              </div>
            </div>
            <div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
                ...
                {fotosObra.map((foto,i)=>(
                  <div key={foto.id} style={{ position:'relative' }}>
                    <ImgUpload
                      ...
                      small
                    />
                    <button onClick={()=>setFotosObra(p=>p.filter(f=>f.id!==foto.id))} style={{ position:'absolute', top:20, right:2, background:'rgba(239,68,68,0.8)', border:'none', borderRadius:4, padding:'2px 5px', color:'#fff', cursor:'pointer', fontSize:10 }}>x</button>
                  </div>
                ))}
                {fotosObra.length===0 && (
                  <div style={{ gridColumn:'span 4', textAlign:'center', padding:32, color:'var(--text-muted)', fontSize:13 }}>
                    Clique em adicionar foto para incluir fotos da obra (max. 12)
                  </div>
                )}
              </div>
            </div>
        ...
        {tab==='patologias' && (
          <div>
            <p style={{ fontSize:12, color:'var(--text-muted)', marginBottom:12 }}>Registre as patologias encontradas. Cada uma pode ter foto e descricao tecnica.</p>
            {patologias.map(pat=>(
        ...
        {tab==='secoes' && (
          <div>
            <p style={{ fontSize:12, color:'var(--text-muted)', marginBottom:12 }}>Cada secao vira uma pagina de servico no PDF com etapas detalhadas e valor.</p>
            {secoes.map(sec=>(
            ...
            <button onClick={()=>setSecoes(s=>[...s,novaSecao()])} style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'var(--text-muted)', padding:'7px 12px', borderRadius:8, border:'1px dashed var(--border)', background:'none', cursor:'pointer' }}>
              <Plus size={13}/> Adicionar secao de servico
            </button>
          </div>
        )}
        ...
        {tab==='financeiro' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div style={{ ...FS, gridColumn:'span 2', background:'var(--bg-hover)', border:'1px solid var(--border)', borderRadius:10, padding:14 }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10 }}>
                {[
                  {l:'Mao de obra', v:fmtR$(totalMaoObra), c:'#60a5fa'},
        ...
        {tab==='condicoes' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div style={{ ...FS, gridColumn:'span 2' }}><label style={LS}>Garantia da execucao</label><textarea value={data.garantia_execucao} onChange={up('garantia_execucao')} style={{ ...TA, minHeight:56 }}/></div>
            <div style={{ ...FS, gridColumn:'span 2' }}><label style={LS}>Observacoes gerais</label><textarea value={data.observacoes} onChange={up('observacoes')} style={TA}/></div>
          </div>
        )}

        {tab==='referencias' && (
          <div>
            <p style={{ fontSize:12, color:'var(--text-muted)', marginBottom:12 }}>Lista de clientes e obras anteriores que aparecem no portfolio do PDF.</p>
            {referencias.map(r=>(
