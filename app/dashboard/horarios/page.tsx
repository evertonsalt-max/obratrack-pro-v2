...
export default function HorariosPage() {
  ...
  const salvar = async () => {
    ...
    try {
      ...
      const diariaFinal = form.usar_diaria_custom && form.diaria_custom
        ? Number(form.diaria_custom)
        : isAus ? 0 : (emp.diaria || 0)

      const payload = {
        employee_id: form.employee_id,
        employee_name: emp.apelido || emp.nome.split(' ')[0],
        data: form.data,
        local: isAus ? '' : form.local,
        jornada: form.jornada,
        entrada: isAus ? null : (form.entrada || null),
        saida: isAus ? null : (form.saida || null),
        horas,
        diaria: diariaFinal,
        vale: false,
        valor_vale: 0,
      ...
      if (editId) {
        ...
        await add(payload)
        if (!isAus && form.local && !worksites.find(w => w.nome === form.local)) {
          addSite(form.local)
        }
      }
      fecharModal()
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={abrirNovo} className="btn-primary"><Plus size={16}/> Novo Registro</button>
      </div>
      ...
      {modal&&(
        <div onClick={e=>{if(e.target===e.currentTarget)fecharModal()}} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
                ...
                <>
                  ...
                  <div className="bg-gray-800 rounded-xl p-4">
                    ...
                    <div className="flex gap-3 mb-3">
                      {[false,true].map(v=>(
                        <button key={String(v)} type="button"
                          onClick={()=>setForm(p=>({...p,usar_diaria_custom:v,diaria_custom:v?p.diaria_custom:''}))}
                          className={`flex-1 py-2 rounded-lg border-2 text-sm font-semibold transition-all ${
                            form.usar_diaria_custom===v
                              ?(v?'border-blue-500 bg-blue-500/15 text-blue-400':'border-green-500 bg-green-500/15 text-green-400')
                    ...
                    {form.usar_diaria_custom?(
                      <div>
                        ...
                        <input type="number" value={form.diaria_custom} onChange={up('diaria_custom')} placeholder="Ex: 250" className="input-field"/>
                      </div>
                    ):(
                      emp&&<p className="text-sm text-gray-400">Diária cadastrada: <span className="text-green-400 font-semibold">{fmtR$(emp.diaria||0)}</span></p>
                    )}
                  </div>
                  ...
                  <div className="bg-gray-800 rounded-xl p-4">
                    ...
                    <div className="flex gap-3 mb-3">
                      {[true,false].map(v=>(
                        <button key={String(v)} type="button"
                          onClick={()=>setForm(p=>({...p,vale:v,discount_value:v?p.discount_value:''}))}
                          className={`flex-1 py-2 rounded-lg border-2 text-sm font-semibold transition-all ${
                            form.vale===v
                              ?(v?'border-green-500 bg-green-500/15 text-green-400':'border-red-500 bg-red-500/15 text-red-400')
                              :'border-gray-600 text-gray-400'
                          }`}>
