...
export async function GET(req: Request) {
  ...
  try {
    ...
    await Promise.all([
      uploadCSV(drive, , toCSV(logs, ['id','employee_id','employee_name','data','jornada','local','entrada','saida','horas','diaria','discount_value','absence_reason','obs'])),
      uploadCSV(drive, , toCSV(employees, ['id','nome','apelido','funcao','diaria','status'])),
      uploadCSV(drive, , toCSV(payments, ['id','employee_id','employee_name','data','valor','tipo','obs'])),
      uploadCSV(drive, , toCSV(taxes, ['id','employee_id','month','year','inss_value','fgts_value','notes'])),
    ])
